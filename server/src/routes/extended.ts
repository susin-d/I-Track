...
router.post("/invitations", requireRole(["admin"]), async (req: AuthRequest, res) => {
  const body = parseOr400(z.object({ name: z.string().min(2), email: z.string().email(), role: z.enum(userRoles).default("engineer") }), req.body, res); if (!body) return;
...
router.patch("/tickets/:id/rank", async (req: AuthRequest, res) => { const body = parseOr400(z.object({ rank: z.number(), sprint: z.string().nullable().optional(), status: z.enum(["Backlog", "To Do", "In Progress", "In Review", "Done"]).optional() }), req.body, res); if (!body) return; const update = { rank: body.rank, ...(body.status && { status: body.status }), ...(body.sprint === null ? { $unset: { sprint: 1 } } : body.sprint ? { sprint: body.sprint } : {}) }; const ticket = await Ticket.findOneAndUpdate({ _id: req.params.id, organization: oid(req) }, update, { new: true }); return ticket ? res.json({ ticket }) : res.status(404).json({ message: "Ticket not found" }); });
