...
const contracts: Record<string, Omit<AiMutationContract, "endpoint">> = {
  ...
  "POST /projects": { body: "{ key: string (2..12), name: string (min 2), description: string (min 5), status?: active|at-risk|done, progress?: number 0..100, riskLevel?: low|medium|high|critical, activeSprint?: string, members?: string[] }" },
  ...
  "POST /resources/:kind": { body: "{ name: string (min 1), project?: string id, key?: string, description?: string, status?: string, order?: number, config?: object }", prerequisites: "kind must be one of the kinds returned by existing resource calls; project must belong to the workspace." },
