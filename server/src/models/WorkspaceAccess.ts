...
export const Invitation = createPgModel({ table: "invitations", columns: ["organization", "email", "name", "role", "capacity", "invitedBy", "tokenHash", "status", "expiresAt", "acceptedBy", "acceptedAt"], defaults: { capacity: 32, status: "pending" }, relations: { organization: { model: () => Organization }, invitedBy: { model: () => User }, acceptedBy: { model: () => User } } });
